...
# -------------------------------------------------
# ШАГ 0: Загрузка выборки
# -------------------------------------------------
# Укажите точное имя вашего файла. Например, 'variant_3.csv'
...
s2 = np.var(data, ddof=1)           # несмещенная дисперсия (ddof=1 делает знаменатель n-1)
...
h = 3.5 * s * n**(-1/3)
# k — количество интервалов (размах делим на ширину и округляем вверх)
...
# а) Гистограмма по правилу Скотта
...
# Получаем данные для гистограммы (высоты и границы интервалов)
counts, bins = np.histogram(data, bins=k, density=True)
# Находим середины интервалов для построения полигона
...
# -------------------------------------------------
# ШАГ 5: Эмпирическая функция распределения (ЭФР)
# -------------------------------------------------
plt.figure(figsize=(8, 5))

# Для ЭФР каждое значение x_i дает скачок 1/n
x_ecdf = sorted_data
...
# Добавляем точки и пунктирные линии (для визуализации скачков)
plt.scatter(x_ecdf, y_ecdf, color='darkmagenta', s=10)
for x, y in zip(x_ecdf[::5], y_ecdf[::5]): # берем каждую 5-ю точку, чтобы не перегружать график
...
# -------------------------------------------------
# ВНИМАНИЕ: Подставьте значения из вашей таблицы вариантов!
# Например, для варианта 3 это могут быть такие числа:
mu_true = 110      # Пример (замените на своё!)
sigma2_true = 144  # Пример (замените на своё!)
